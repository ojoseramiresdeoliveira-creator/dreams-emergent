...
export function useVideoScrub({ videoRef, trackRef, enabled = true } = {}) {
  ...
  useEffect(() => {
    ...
    // Build once we have the clip's duration.
    const onMeta = () => build();
    const start = () => {
      if (video.readyState >= 1 /* HAVE_METADATA */) build();
      else video.addEventListener('loadedmetadata', onMeta, { once: true });
    };

    // Lazy load: paired with preload="none", the clip costs zero bytes until
    // its track nears the viewport (one screen ahead). Only then do we fetch it
    // and wire the scrub. Mobile / reduced motion returned above, so those
    // never load a clip at all — they stay on the poster.
    let io = null;
    if (video.readyState >= 1) {
      start(); // already cached — no need to wait for the observer
    } else {
      io = new IntersectionObserver(([entry]) => {
        if (!entry.isIntersecting) return;
        io.disconnect();
        io = null;
        video.load(); // begin fetching → loadedmetadata → build
        start();
      }, { rootMargin: '100% 0px' });
      io.observe(track);
    }
